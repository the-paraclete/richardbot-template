...
The framework reaches two AI surfaces from one source, with the **same**
behavioral discipline on both — only the delivery mechanism differs.

- **Claude Code** gets it automatically. The honesty/completion rules and the
  memory protocol load every turn via `alwaysApply` fragments + the
  `prompt-fragment-loader.sh` hook; runtime hooks gate destructive commands.
- **GitHub Copilot** gets the identical discipline from one always-loaded file,
  `.github/copilot-instructions.md`. A bare `richardbot-init init` scaffolds it
  immediately (no `--full` needed). The file has two regions split by a sentinel:

  ```
  <static discipline+memory preamble>            ← preserved across re-runs; hand-editable
  <!-- BEGIN richardbot-mirror — generated; everything below is overwritten on re-run -->
  <folded rules + memory "Known findings">       ← regenerated by copilot-mirror
  ```

  The preamble (sourced from `assets/copilot-preamble.md`) carries the
  honesty/completion rules — *"Never claim an action you did not take"*, *"A plan
  is not a deliverable"* — and the manual memory protocol Copilot runs by hand:
  read `.richardbot-memory/_recent.md` at session start; append a note + prepend
  a digest line when it learns something. `copilot-mirror` regenerates only the
  region **below** the sentinel, so hand edits to the preamble survive every run.

Where Claude Code's hook does the memory read/write automatically, Copilot does
it manually — same files, same `_recent.md` digest format, one behavioral spec.
